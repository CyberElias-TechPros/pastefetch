...
> **v2 addendum (free-tier pivot).** This document is the original v1 plan (paid-agnostic).
> The build in this repo implements a **free-tier edition** — the strategic changes are:
>
> 1. **Hetzner boxes → free container host for the resolver.** The Python/yt-dlp worker becomes an isolated *resolver microservice* (`resolver/`) deployable to a free container platform (Hugging Face Space, Koyeb, Render free, or Oracle Cloud Always Free). Same zero-storage rules.
> 2. **FastAPI monolith API → Cloudflare Worker (free tier).** The API, job store, and rate limiting move to a Worker + D1, with *native TypeScript extractors* (`worker/src/extractors/`) for platforms with public JSON endpoints so the core product works with **zero** container infrastructure.
> 3. **Stream-through everything → redirect-first.** Free tiers have no bandwidth budget to spare. The download endpoint 302s the browser straight to the platform CDN whenever the URL isn't IP-locked; only IP-locked URLs (YouTube) stream through the resolver's egress. See `docs/FREE-TIER-GUIDE.md` for limits, capacity math and the upgrade path.
> 4. **Residential proxy pool → BYO-cookies + honest status page.** Paid proxy spend is off the table on a free budget; the canary cron + public `/api/status` page keeps the "honest reliability" brand promise, and the resolver accepts a user-supplied cookies file for YouTube.
>
> Everything below (product vision, UX principles, legal posture, roadmap, metrics) still applies.
